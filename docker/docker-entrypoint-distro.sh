...
# from https://raw.githubusercontent.com/docker-library/wordpress/master/php7.3/apache/docker-entrypoint.sh
# see #STOREKEER: to see the changes
...
#STOREKEER: added  [[ "$1" == docker-entrypoint.sh ]] || is the only change so we can still use this file mainly unchanged, and it can be proxied to out reall entrypoint
if [[ "$1" == docker-entrypoint.sh ]] || [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	if [ "$(id -u)" = '0' ]; then
		case "$1" in
			apache2*)
