...
echo '<label for="agree">
        <input type="checkbox" name="agree" id="agree" checked disabled> <strong>' . __(
        'Required Image', I18N::DOMAIN
    ) . '</strong>
      </label>';
...
foreach ($addon['options'] as $option) {
    ...
    echo '<input type="hidden" id="uploaded_image_url_' . esc_attr($option['id']) . '" 
        name="addon_image[' . esc_attr($addon['product_addon_group_id']) . '][' . esc_attr($option['id']) . '][' . esc_attr(ProductAddOnHandler::ADDON_TYPE_REQUIRED_IMAGE) . ']" 
