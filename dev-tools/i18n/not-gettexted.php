...
class NotGettexted
{
    ...
    public $commands = ['extract' => 'command_extract', 'replace' => 'command_replace'];
    ...
    public function make_string_aggregator($global_array_name, $filename)
    {
        ...
        return create_function(
            '$string, $comment_id, $line_number',
            'global $'.$a.'; $'.$a.'[] = array($string, $comment_id, '.var_export(
                $filename,
                true
            ).', $line_number);'
        );
    ...
    public function make_mo_replacer($global_mo_name)
    {
        ...
        return create_function(
            '$token, $string',
            'global $'.$m.'; return var_export($'.$m.'->translate($string), true);'
        );
    }
    ...
    public function command_extract()
    {
        ...
        foreach ($filenames as $filename) {
            ...
            $this->walk_tokens($tokens, [$this, 'ignore_token'], [$this, 'ignore_token'], $aggregator);
        }
        ...
        foreach ($GLOBALS[$global_name] as $item) {
            ...
            $args = [
                ...
                'references' => ["$filename$ref_line_number"],
            ];
    ...
    public function command_replace()
    {
        ...
        foreach ($filenames as $filename) {
            ...
            $new_file = $this->walk_tokens($tokens, $replacer, [$this, 'unchanged_token']);
            $f = fopen($filename, 'w');
            fwrite($f, $new_file);
            fclose($f);
        }
    ...
    public function cli()
    {
        ...
        call_user_func_array([$this, $this->commands[$argv[1]]], array_slice($argv, 2));
    }
}
